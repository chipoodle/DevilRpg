...
public class SkillSummonWispForester extends AbstractSkillExecutor {

    public SkillSummonWispForester(PlayerSkillCapabilityImplementation parentCapability) {
        super(parentCapability);
    }

    @Override
    public SkillEnum getSkillEnum() {
        return SkillEnum.SUMMON_WISP_FORESTER;
    }
    ...
    public void execute(Level level, Player player, HashMap<String, String> parameters) {
        if (!player.getCooldowns().isOnCooldown(icon.getItem())) {
            if (!level.isClientSide) {
                ...
                minionCap.summonWispComplete(level, player, rand, () -> summonWisp(level, player, rand), maxSummons, SoulWispForester.class);
            }
        }
    ...
    private SoulWisp summonWisp(Level levelIn, Player playerIn, Random rand) {
        ...
        SoulWispForester sw = ModEntities.WISP_FORESTER.get().create((ServerLevel) levelIn, null, null, blockPos, MobSpawnType.MOB_SUMMONED, true, true);
