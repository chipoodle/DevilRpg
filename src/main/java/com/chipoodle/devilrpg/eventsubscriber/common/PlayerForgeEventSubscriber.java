...
public class PlayerForgeEventSubscriber {
	...
	public static void onPlayerLogsIn(PlayerEvent.PlayerLoggedInEvent event) {
		...
		String message2 = String.format("You have skills.");
	...
	public static void onEntityJoinWorld(EntityJoinWorldEvent event) {
		if (event.getEntity() instanceof PlayerEntity) {
			...
			LazyOptional<IBaseManaCapability> mana = player.getCapability(PlayerManaCapabilityProvider.MANA_CAP);
			LazyOptional<IBaseSkillCapability> skill = player.getCapability(PlayerSkillCapabilityProvider.SKILL_CAP);
			int capSoulWolf = skill.map(x->x.getPaSoulWolf()).orElse(0);
			int maxSoulWolf = skill.map(x->x.getMaxSoulWolf()).orElse(0);
			
			Minecraft mainThread = Minecraft.getInstance();
			if (!mana.isPresent())
				DevilRpg.LOGGER.info("|||------>onEntityJoinWorld " + player.getEntityId() + " cap = null" + " PLayer remote? "
						+ player.world.isRemote);
			else {
				DevilRpg.LOGGER.info("|||------>onEntityJoinWorld " + player.getEntityId() + " cap = " + mana
						+ " Cliente? " + player.world.isRemote + " Mana: " + mana.map(x->x.getMana()).orElse(0.0f) + " MaxMana: "
						+ mana.map(x->x.getMaxMana()).orElse(0.0f));
				if (!player.world.isRemote) {
					mainThread.enqueue(new Runnable() {
						public void run() {
							DevilRpg.LOGGER.info("||----------- ON JOIN ENVIANDO CAP DE SERVIDOR A CLIENTE: "+((ServerPlayerEntity) player).getName());
							ModNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(() ->(ServerPlayerEntity) player), new PlayerManaClientServerHandler(mana.map(x -> x.getNBTData()).orElse(null)));
						}
					});
				}
			}
			if (!skill.isPresent())
				...
				DevilRpg.LOGGER.info("|||------>onEntityJoinWorld " + player.getEntityId() + " cap = " + skill
						+ " Cliente? " + player.world.isRemote + " getPaSoulWolf: " + capSoulWolf
						+ " getMaxSoulWolf: " +maxSoulWolf);
				if (!player.world.isRemote) {
					mainThread.enqueue(new Runnable() {
						public void run() {
							DevilRpg.LOGGER.info("||----------- ON JOIN ENVIANDO CAP DE SERVIDOR A CLIENTE: "+((ServerPlayerEntity) player).getName());
							ModNetwork.CHANNEL.send(PacketDistributor.PLAYER.with(() ->(ServerPlayerEntity) player), new PlayerSkillClientServerHandler(skill.map(x -> x.getNBTData()).orElse(null)));
						}
					});
				}
			}
