...
public abstract class AbstractMountableTameableEntity extends Animal implements ContainerListener, PlayerRideableJumping, Saddleable {
    ...
    private static final Predicate<LivingEntity> PARENT_HORSE_SELECTOR = (p_213617_0_) -> {
        return p_213617_0_ instanceof AbstractMountableTameableEntity && ((AbstractMountableTameableEntity) p_213617_0_).isBred();
    };
    ...
    private static final DataParameter<Optional<UUID>> DATA_ID_OWNER_UUID = EntityDataManager.defineId(AbstractMountableTameableEntity.class, DataSerializers.OPTIONAL_UUID);
    ...
    protected AbstractMountableTameableEntity(EntityType<? extends AbstractMountableTameableEntity> p_i48563_1_, World p_i48563_2_) {
    ...
    public static AttributeModifierMap.MutableAttribute createBaseHorseAttributes() {
        return MobEntity.createMobAttributes().add(Attributes.JUMP_STRENGTH).add(Attributes.MAX_HEALTH, 53.0D).add(Attributes.MOVEMENT_SPEED, 0.225F);
    }
    ...
    public void openInventory(PlayerEntity player) {
        if (!this.level.isClientSide && (!this.isVehicle() || this.hasPassenger(player)) && this.isTame()) {
            openHorseInventory(this, this.inventory, (ServerPlayerEntity) player);
        }

    }

    public void openHorseInventory(AbstractMountableTameableEntity entity, IInventory inventory, ServerPlayerEntity player) {
        if (player.containerMenu != player.inventoryMenu) {
            player.closeContainer();
        }

        player.nextContainerCounter();
        player.connection.send(new SOpenHorseWindowPacket(player.containerCounter, inventory.getContainerSize(), entity.getId()));
        player.containerMenu = new TameableMountableHorseInventoryContainer(player.containerCounter, player.inventory, inventory, entity);
    ...
    protected void followMommy() {
        if (this.isBred() && this.isBaby() && !this.isEating()) {
            LivingEntity livingentity = this.level.getNearestEntity(AbstractMountableTameableEntity.class, MOMMY_TARGETING, this, this.getX(), this.getY(), this.getZ(), this.getBoundingBox().inflate(16.0D));
