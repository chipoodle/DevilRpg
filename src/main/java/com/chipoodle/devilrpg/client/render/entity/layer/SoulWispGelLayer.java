...
public class SoulWispGelLayer<T extends SoulWispEntity> extends GhostEnergyLayer<T, SoulWispModelHeart<T>> {
	...
	// private static final ResourceLocation WISP_GEL = new
	...
	public void render(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, T entitylivingbaseIn,
			...
			float headPitch) {
		if (entitylivingbaseIn != null && entitylivingbaseIn.getOwner() != null) {
			// renderWings(matrixStackIn, bufferIn, packedLightIn, entitylivingbaseIn,
	...
	/*
	 * private void renderWings(MatrixStack matrixStackIn, IRenderTypeBuffer
	 * bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float
	 * limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
	 * matrixStackIn.push(); matrixStackIn.scale(4.5f, 4.5f, 4.5f);
	 * matrixStackIn.translate(0.0f, -1.1f, 0.0f); IVertexBuilder ivertexbuilder =
	 * bufferIn.getBuffer(this.wingsModel.getRenderType(WISP_WINGS));
	 * this.wingsModel.render(matrixStackIn, ivertexbuilder, packedLightIn,
	 * OverlayTexture.DEFAULT_LIGHT, 1.0F,1.0F, 1.0F, 1.0F);
	 * this.wingsModel.render(entitylivingbaseIn, limbSwing, limbSwingAmount,
	 * ageInTicks, netHeadYaw, headPitch); matrixStackIn.pop(); }
	 */
	...
	private void groovyMethod(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn,
		...
		if (!entitylivingbaseIn.isInvisible()) {
			...
			float[] rgbArray = IRenderUtilities.groovyBlue(entitylivingbaseIn, partialTicks);
