...
@OnlyIn(Dist.CLIENT)
public class WerewolfItemInHandRenderer extends ItemInHandRenderer {
    ...
    public WerewolfItemInHandRenderer(Minecraft p_234241_, EntityRenderDispatcher p_234242_, ItemRenderer p_234243_) {
    ...
    static WerewolfItemInHandRenderer.HandRenderSelection evaluateWhichHandsToRender(LocalPlayer p_172915_) {
      ...
      if (!flag && !flag1) {
         ...
         return isChargedCrossbow(itemstack) ? WerewolfItemInHandRenderer.HandRenderSelection.RENDER_MAIN_HAND_ONLY : WerewolfItemInHandRenderer.HandRenderSelection.RENDER_BOTH_HANDS;
      }*/
    }

    private static WerewolfItemInHandRenderer.HandRenderSelection selectionUsingItemWhileHoldingBowLike(LocalPlayer p_172917_) {
        ...
        if (!itemstack.is(Items.BOW) && !itemstack.is(Items.CROSSBOW)) {
            return interactionhand == InteractionHand.MAIN_HAND && isChargedCrossbow(p_172917_.getOffhandItem()) ? WerewolfItemInHandRenderer.HandRenderSelection.RENDER_MAIN_HAND_ONLY : WerewolfItemInHandRenderer.HandRenderSelection.RENDER_BOTH_HANDS;
        } else {
            return WerewolfItemInHandRenderer.HandRenderSelection.onlyForHand(interactionhand);
        }
    }
    ...
    @Override
    public void renderHandsWithItems(float partialTicks, PoseStack poseStack, MultiBufferSource.BufferSource bufferSource, LocalPlayer player, int packedLight) {
        ...
        if (iteminhandrenderer$handrenderselection.renderMainHand) {
            ...
            this.renderArmWithItem(player, partialTicks, f1, InteractionHand.MAIN_HAND, f4, this.mainHandItem, f5, poseStack, bufferSource, packedLight);
        }
        ...
        if (iteminhandrenderer$handrenderselection.renderOffHand) {
            ...
            this.renderArmWithItem(player, partialTicks, f1, InteractionHand.OFF_HAND, f6, this.offHandItem, f7, poseStack, bufferSource, packedLight);
        }

        bufferSource.endBatch();
    }

    private void renderArmWithItem(AbstractClientPlayer abstractClientPlayer, float partialTicks, float p_109374_, InteractionHand interactionHand, float p_109376_, ItemStack itemStack, float p_109378_, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        if (!abstractClientPlayer.isScoping()) {
            ...
            if (itemStack.isEmpty()) {
                this.renderPlayerArm(poseStack, bufferSource, packedLight, p_109378_, p_109376_, humanoidarm);
                ...
                if (flag && this.offHandItem.isEmpty()) {
                    this.renderTwoHandedMap(poseStack, bufferSource, packedLight, p_109374_, p_109378_, p_109376_);
                } else {
                    this.renderOneHandedMap(poseStack, bufferSource, packedLight, p_109378_, humanoidarm, p_109376_, itemStack);
                }
                ...
                if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand) {
                    this.applyItemArmTransform(poseStack, humanoidarm, p_109378_);
                    ...
                    float f9 = (float) itemStack.getUseDuration() - ((float) this.minecraft.player.getUseItemRemainingTicks() - partialTicks + 1.0F);
                ...
                this.renderItem(abstractClientPlayer, itemStack, flag2 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, !flag2, poseStack, bufferSource, packedLight);
            } else {
                boolean flag3 = humanoidarm == HumanoidArm.RIGHT;
                if (!net.minecraftforge.client.extensions.common.IClientItemExtensions.of(itemStack).applyForgeHandTransform(poseStack, minecraft.player, humanoidarm, itemStack, partialTicks, p_109378_, p_109376_)) // FORGE: Allow items to define custom arm animation
                    if (abstractClientPlayer.isUsingItem() && abstractClientPlayer.getUseItemRemainingTicks() > 0 && abstractClientPlayer.getUsedItemHand() == interactionHand) {
                        int k = flag3 ? 1 : -1;
                        switch (itemStack.getUseAnimation()) {
                            ...
                            case SPEAR:
                                ...
                                break;
                            case BRUSH:
                                this.applyBrushTransform(poseStack, partialTicks, humanoidarm, itemStack, p_109378_);
                        }
                ...
                this.renderItem(abstractClientPlayer, itemStack, flag3 ? ItemDisplayContext.FIRST_PERSON_RIGHT_HAND : ItemDisplayContext.FIRST_PERSON_LEFT_HAND, !flag3, poseStack, bufferSource, packedLight);
            }
