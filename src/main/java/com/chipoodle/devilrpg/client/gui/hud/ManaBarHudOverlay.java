...
public class ManaBarHudOverlay {
    ...
    private final static ResourceLocation overlayBar = ResourceLocation.fromNamespaceAndPath(DevilRpg.MODID, "textures/gui/mana_texture.png");
    ...
    public static final LayeredDraw.Layer HUD_MANA_BAR = (guiGraphics, deltaTracker) -> {
        Minecraft mc = Minecraft.getInstance();
        Player player = mc.player;
        if (player == null || player.isCreative())
            return;

        PlayerManaCapabilityInterface playerCapability = player.getData(PlayerManaCapability.INSTANCE);
        if (playerCapability == null) return;

        float maxMana = playerCapability.getMaxMana();
        float manaRun = playerCapability.getMana();
        Font fr = mc.font;
